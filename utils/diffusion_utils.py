...
def model_fn(model, x, t, y=None,class_cond=True):
    assert y is not None
    return model(x, t, y if class_cond else None)

def denoising_step_cond(xt, t, t_next, *,
                   models,
                   logvars,
                   b,
                   sampling_type='ddpm',
                   ...
                   hybrid_config=None,
                   ratio=1.0,
                   out_x0_t=False,cond_fn = None,classifier,y
                   ):
    ...
    xt_next = torch.zeros_like(xt)
    gradient = cond_fn(classifier,xt, t,y,1.0)
    assert sampling_type == 'ddpm'
    if sampling_type == 'ddpm':
        weight = bt / torch.sqrt(1 - at)
        ...
        mask = 1 - (t == 0).float()
        mask = mask.reshape((xt.shape[0],) + (1,) * (len(xt.shape) - 1))
        xt_next = mean + mask * torch.exp(0.5 * logvar) * noise * gradient
        xt_next = xt_next.float()
