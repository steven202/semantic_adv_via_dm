...
model.load_state_dict(torch.load(save_path, map_location=torch.device(f"cuda"))["net"])
...
with torch.no_grad():
    for folder in tqdm(sorted(os.listdir(ori_path))):
        model.eval()
        test_loss = 0
        success = 0
        total = 0
        try:
            ...
            files = sorted(os.listdir(os.path.join(ori_path, folder)))
            for f in tqdm(files):
                if len(re.findall(r"\d+", f)[0]) != 0:
                    id = int(re.findall(r"\d+", f)[0])
                    image = custom_transforms(
                        Image.open(os.path.join(ori_path, folder, f)).convert("RGB")
                    ).unsqueeze(0)
            ...
            print(
                f"Original Loss: {test_loss / (total):.4f} | ASR: {100.0 * success/total:.4f} % | ({success}/{total})"
            )
            break
        except:
            continue
...
"""
rm classifier_logs/pgd_l2_defense.txt
CUDA_VISIBLE_DEVICES=0 python test_asr_pgd.py --model 5 >> classifier_logs/pgd_l2_defense.txt
"""
