...
from diffusionclip_afhq_train import DiffusionCLIP
...
def parse_args_and_config():
    ...
    if args.clip_finetune or args.clip_finetune_eff:
    ...
    elif args.attack or args.example:
        if args.model_path:
            args.exp = (
                args.exp
                + f'_mask{args.mask}_diff{args.diff}_eta{args.eta}_renorm{args.renorm}_smooth{args.smooth}_ED_{new_config.data.category}_t{args.t_0}_ninv{args.n_inv_step}_ngen{args.n_train_step}_{os.path.split(args.model_path)[-1].replace(".pth","")}'
            )
        elif args.hybrid_noise:
            hb_str = "_"
            for i, model_name in enumerate(HYBRID_MODEL_PATHS):
        ...
        args.exp = (
            args.exp + f"_FOpt_{new_config.data.category}_{args.trg_txts[0]}_t{args.t_0}_ninv{args.n_train_step}"
        )
